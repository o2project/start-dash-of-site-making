...
これをラブライブ参加者募集サイトの見た目に合わせるため、`min-height: 0;`と指定して、グローバルナビゲーションの高さを狭めておきます。
...
次に選択できないことを示すため、`pointer: default;`としてマウスカーソルの形を矢印にします。
...
#ll-nav .ll-active > a:focus:after,
#ll-nav .ll-active > a:hover:after {
  content: "";
  display: block;
  position: absolute;
  bottom: 0;
