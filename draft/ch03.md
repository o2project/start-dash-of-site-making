...
ファイル構成は次のとおりとなります。ディレクトリーとしては`components`・`styles`・`scripts`・`images`の4つがあり、その中に各ファイルを置く想定です。
...
また、現在見ているページを示すためのクラス名として、Top部分に`active`というクラス名を付けています。
`active`に関しては、クラス名に接頭辞を付けなくて大丈夫です。
...
さらに元のデザインに合わせるため、グローバルナビゲーションで現在開いているページを示すセレクター（`.active`）に対し、背景色や前景色を指定します。
