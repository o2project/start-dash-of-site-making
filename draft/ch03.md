...
[HTML Standard - 12.1.1 The DOCTYPE](https://html.spec.whatwg.org/#the-doctype) では、`<!DOCTYPE` について「大文字小文字問わず」と書かれています。つまり `<!doctype` と書いても問題ありません。
...
なお lory の読み込み時に、`defer` を指定しています。`defer` は「非同期にファイルを読み込む」という意味の HTML 属性です。`defer` は `DOMContentLoaded` という **HTML が全て読み込まれたタイミングで実行されるイベント** の前に実行されます。
...
これらのクラスについては「JavaScript で動きを付ける」で説明します。
