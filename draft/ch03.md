...
[8.1.1 DOCTYPE — HTML 5.1 日本語訳](http://momdo.github.io/html51/syntax.html#the-doctype)では、`<!DOCTYPE` について「大文字小文字問わず」と書かれています。
...
`main`要素へ`role`属性の値に`main`という値を付けます。これは仕様書の[4.4.14 main要素](http://momdo.github.io/html51/grouping-content.html#the-main-element)にも「書くことを勧める」と書かれています。
なお、`role`属性に指定した`main`という値は、[サイトの主要な内容](http://momdo.github.io/wai-aria-1.1/#main)を指しています。一見した場合`main`要素と用途がかぶっているように見えます。
しかし、Internet Explorerが[IE11まではmain要素をサポートしていない](http://caniuse.com/#feat=html5semantic)ため、`role`属性で`main`という値も一緒に書く必要があります。
