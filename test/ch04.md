...
今回カルーセルの実装にslickというライブラリを使います。slick用としてリスト2のHTMLを用意します。とはいっても切り替え表示させる画像を`js-slides ll-slides`というクラスを付けた`div`要素で囲むだけです。接頭辞に`js-`と付けているクラスはJavaScriptから触ることを明確にするため付けています（リスト2）。
