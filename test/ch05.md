...
ライブステージページの`main`要素に対し、要素の内側の余白を指定する`padding`や背景色を指定する`background-color`を指定することでライブステージページの見た目に近づけていきます。さらに見出しにもスタイルを適用することでサイトの見た目をアニメ版に近づけていきます（リスト7）。
