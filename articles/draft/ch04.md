...
第3章では共通の構造部分を作っていきました。そこではヘッダーやフッターなどはマークアップしましたが、`main`要素の中身は空だったと思います。
この章では空のままだった`main`要素の中をHTMLでマークアップしていきます。
...
その左横の数字を消すために`.ll-slides-pagination`に対して`list-style-type: none;`と指定して数字を無くします。
