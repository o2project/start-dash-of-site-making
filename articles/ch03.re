...
いよいよ、サイトを作っていきます。
まずはサイト共通の構造をトップページとライブステージページから洗い出し、それをマークアップしていきます。

== 共通の構造を洗い出し

トップページ（@<img>{lovelive-screenshot-top}）やライブステージページ（@<img>{lovelive-screenshot-livestage}）のアニメキャプチャ画像から共通構造を抜き出すと、次に挙げる3つが共通構造となります。

 * ヘッダー
 * グローバルナビゲーション
 * フッター

これら共通構造にサイトの各ページへたどれるリンクを置くことで、どのページからもサイト内にあるページへ行けます。
...
このHTMLを基礎として肉付けしていきます（@<list>{base-html}）。

//list[base-html][基本となるHTML]{
<!DOCTYPE html>
<html lang="ja">
<head>
  <meta charset="UTF-8">
...
今回はCSSライブラリとしてBootstrapのテーマであるNicoを使っているため、各要素の@<tt>{class}属性にBootstrapで定義されているクラス名を定義します（@<list>{global-header}）。

//list[global-header][ヘッダーのHTML]{
...
作らないページについては、@<list>{no-link}のように@<tt>{a}要素の@<tt>{href}属性に@<tt>{#}を入れておきます。
こうすることで、作っていないページの検索が楽になるのを狙っています。

//list[no-link][作らないページはhref属性の値に#を入れておく]{
<a href="#">Mail magazine</a>
//}

Bootstrapにはないクラス名を定義する際、@<list>{ll}のように@<tt>{ll-}という接頭辞を付けています（@<strong>{l} ove @<strong>{l} ive）。
これはBootstrapで定義されているクラス名と競合しないように、安全策で接頭辞を付けています。

//list[ll][ll-という接頭辞を付けている]{
<h1 class="navbar-brand ll-logo">LoveLive!</h1>
//}
...
@<tt>{ll-active}内でBootstrapが@<tt>{active}に対してスタイル定義しているところを上書きします（@<list>{global-navigation}）。

//list[global-navigation][グローバルナビゲーションのHTML]{
...
しかしフッター内にページネーションを含めるのは一般的でないため、特にマークアップしません（@<list>{global-footer}）。

//list[global-footer][フッターのHTML]{
...
ここまでをまとめると、@<list>{toppage-html}のHTMLになります。

//list[toppage-html][共通部分のHTML]{
<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
...
スクリーンショットは@<img>{common-html}のとおりになります。
...
これは要素の高さ調整と@<tt>{margin}・@<tt>{padding}を使って要素内外にある隙間を調整すれば、間延びした見た目ではなくなります（@<list>{header-css}）。

//list[header-css][ヘッダーのCSS]{
/* サイトのヘッダー */
#ll-header {
  margin-bottom: 0;
}
...
Bootstrapのスタイル定義をそのまま使った場合、グローバルナビゲーションは高さがあります。
これをラブライブ！参加者募集サイト用に、@<tt>{min-height: 0;}と指定して、グローバルナビゲーションの高さを狭めておきます。

またグローバルナビゲーション内の文字が全て大文字で書かれています。
そのためグローバルナビゲーション内の要素に対し@<tt>{text-transform: uppercase;}と指定して、文字を全て大文字にします。
他のスタイル定義に関しては、Bootstrapで定義されているスタイルを打ち消すための指定となります（@<list>{global-navigation-css}）。

//list[global-navigation-css][グローバルナビゲーションのCSS]{
/* サイトのグローバルナビゲーション */
#ll-nav {
  min-height: 0;
  margin-bottom: 0;
...
元のデザインに合わせるため、現在開いているページを示すセレクター（@<tt>{.ll-active}）に対し背景色や前景色を指定します。
さらにリンクをマウスカーソルで押したらリンク先に飛ぶというマウスイベントが起こらないよう、@<tt>{pointer-events: none;}の指定でマウスイベント無効化をします（@<list>{global-navigation-css-03}）。
最後に選択できないことを示すため@<tt>{cursor: default;}としてマウスカーソルの形を矢印にします。

//list[global-navigation-css-03][現在見ているページはクリックできないようにする]{
...
現在開いているページをグローバルナビゲーション上で分かりやすくするため、下部に黄色の線を引きます（@<list>{global-navigation-css-04}）。

//list[global-navigation-css-04][グローバルナビゲーション上で現在見ているページを分かりやすくする]{
...
最後にフッターへスタイルを適用します。
Bootstrapで定義されている要素外側の余白を打ち消します（@<list>{global-footer-css}）。

//list[global-footer-css][フッターのCSS]{
/* サイトのフッター */
#ll-footer {
  margin-bottom: 0;
}
...
ここまでをまとめると、@<list>{common-css}のCSSになります。

//list[common-css][共通部分のCSS]{
/* サイトのヘッダー */
#ll-header {
  margin-bottom: 0;
}
...
スクリーンショットは@<img>{common-elements}のとおりになります。
