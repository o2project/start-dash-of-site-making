...
そのため拡張性はエディターのほうが高いです。その分エディターの設定に時間がかかります。
...
そのときGitに作業履歴が残っていれば、かなり前の作業まで遡ることが少なくなります。
...
「CSSで背景色をつけるやり方」と検索するより、「CSS 背景色 やり方」のように単語で区切るほうが検索結果の件数は増えます。
検索エンジンは「CSSで背景色をつけるやり方」のような文章が苦手です。
そのため文章よりも単語ごとに区切ったほうが検索結果の件数は増えます。

==== 「○○ とは」で検索する

より入門的な内容を探したい場合「○○ とは」で検索するのがいいです。
理由は入門的な記事のタイトルは「○○ とは」のように書く傾向があるためです。
たとえばJavaScriptの初心者向け記事を探したい場合「JavaScript とは」といった感じで検索します。

==== エラーメッセージで検索する

JavaScriptでエラーメッセージが出力された場合、エラーメッセージをそのまま検索すれば解決方法が出てきやすいです。
しかし大部分が英語のため、そこは頑張って内容を読み取るしかないです。

==== 期間を指定して検索する

Google検索結果内の「検索ツール - 期間指定」を使ってなるべく新しい情報のみ検索します。
CSSやJavaScriptは2011年以前の情報があまり役に立ちません。そのため、期間を絞り新しい情報にたどりつきやすくします。
...
この拡張機能を使えば、期間指定に「3ヶ月以内」と「6ヶ月以内」が増えます。
...
この「○○」に入る言葉は、たとえば「ある機能の実装方法」が入ると思います。
...
今回作成するサイトのファイル構成を考えると@<list>{file-structure}になります。
...
たとえばCSSだとBootstrap、JavaScriptだとjQueryなどがライブラリです。
...
利用実績で見るのは公開されてからある程度時間が経過し、広く使われるライブラリのみ使える判断材料です。
つまり定番と言われるライブラリに対してのみ判断材料として使えます。

そこでIssueやPull Requestの件数と最新コミット日時を見ます。
たとえば最新コミット日が6か月前だった場合、IssueやPull Requestの多さは「放置されているから」という理由になります。
...
ラブライブ！参加者募集サイトは、Bootstrapを使って作られたサイトの見た目に似ているためBootstrapを使います。
...
ふたつめとして、slickは開発が活発なためです。ただ互換性がない更新も起こるので最新版に更新する際は注意が必要です。
