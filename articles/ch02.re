...
この章では、ラブライブ!募集サイトの実装にあたって、どんなことを事前に準備していけばいいかを書いていきます。
...
ひとつめに必要なものとして、ソースコードを書くためにエディターやIDE（統合開発環境）を用意します。
...
ふたつめに必要なものとして、作ったサイトの表示確認用にブラウザーを用意します。
...
まずひとつめの手法として、たとえば@<tt>{CSSで背景色をつけるやり方}と検索するより、@<tt>{CSS 背景色 やり方}で検索するほうが検索結果の件数は増えます。
検索エンジンは@<tt>{CSSで背景色をつけるやり方}のような文章が苦手です。
そのため、文章よりも単語ごとに区切ったほうが検索結果の件数は増えます。



ふたつめの手法として、より入門的な内容を探したい場合、@<tt>{○○ とは}で検索するのが良いです。
...
ひとつめの基準として、そのライブラリが広く使われているかを見ます。
...
ふたつめの基準として、GitHub上にあるIssueやPull Requestの件数と最新コミット日時です。
...
選定した理由としては、今回デザイナーがいない中、ラブライブ!参加者募集サイトの見た目をアニメに近づけたいという思いがありました。
ラブライブ!参加者募集サイトは、Bootstrapを使って作られたサイトの見た目に似ているため、Bootstrapを使うことにしました。



また、ラブライブ!参加者募集サイトはピンク系の配色なのですが、Bootstrap本体はピンク系の配色ではありません。
...
選定した理由はふたつあります。
理由のひとつめとして、外部ライブラリに依存せず、loryだけを読みこめばカルーセルの実装をおこなえることが挙げられます。
理由のふたつめとして、loryは全体像が把握しやすいことです。
