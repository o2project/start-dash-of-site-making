...
=== ソースソースコードの全体が把握できる、もしくは、ドキュメントが揃っている


次に、ソースソースコードの全体が把握しやすいか、もしくは、ドキュメントが揃っていてそれが読みやすいかどうかを見ます(もちろん両方揃っているのが好ましいです)。



例えば、ソースソースコードの全体が把握しやすいライブラリとしては、@<href>{http://backbonejs.org/,Backbone.js}が挙げられます。
...
しかし、@<href>{https://github.com/jashkenas/backbone/blob/master/backbone.js,ソースソースコード}はまとまりの単位が小さいため、1,877行(2015/7/10現在)あるとはいえ、全体を読もうと思えば読めなくはないものとなっています。
...
ソースソースコード自体は @<href>{https://github.com/jquery/jquery/tree/master/src,リポジトリの src ディレクトリ配下}にまとまっており、機能ごとにまとまってはいるのですが、ファイル数が多く全体を把握しようとすると骨が折れます。



ただ、@<href>{https://api.jquery.com/,ドキュメント}が見やすくまとまっており、普段はソースソースコードを見なくてもどのように使うのか調べるのに困らないという状況になっています。
...
また@<href>{https://github.com/hokaccha/js-flipsnap/blob/master/flipsnap.js,ソースソースコード自体}も、まとまりの単位が小さく、読みやすいものとなっています。
 ...
 * ライブラリのソースソースコードの全体が把握しやすい(@<href>{https://github.com/meandmax/lory/blob/73244bb780e0214d7c4b446d27295a80c6872bd0/dist/lory.js,2015/7/10現在で520行})
