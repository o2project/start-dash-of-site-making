...
この章ではラブライブ!募集サイトを実装する前の準備について書きます。


== 必要なツールの準備


はじめに、ラブライブ!募集サイトの実装で必要なツールを準備します。
...
手法のひとつ目として@<tt>{CSSで背景色をつけるやり方}と検索するより、@<tt>{CSS 背景色 やり方}のように単語で区切るほうが検索結果の件数は増えます。
...
手法の3つ目として、Google検索結果内にある@<tt>{検索ツール - 期間指定}を使ってなるべく新しい情報のみ検索します。
