 ...
 * @<href>{http://qiita.com/tanakh/items/a1b4705fb58ef3c89800,Windows - 開発者がSurfacePro3を買ったらまずやること - Qiita}
 * @<href>{http://tanakh.jp/posts/2013-05-23-windows-setup.html,最近のWindowsの開発環境のセットアップ - 純粋関数空間}


ターミナルは「黒い画面」とも呼ばれ、時には恐れられる存在になりますが、以下に紹介する記事を見ていただければ、触りにくいという考えは多少は軽減されると思います。

 * @<href>{http://qiita.com/tokimari/items/43b89c832812ea2bd93f,Terminal - デザイン系フロントエンドエンジニアが「黒い画面」を使ってて良かったこと。 - Qiita}
 * @<href>{http://fjord.jp/love/548.html,Webデザイナーの為の「本当は怖くない」”黒い画面”入門 Part.01 « 合同会社フィヨルド}
...
そこで Google が公開している @<href>{https://developers.google.com/web/tools/starter-kit/,Web Starter Kit} を使いましょう。



使い方や開発方法などは、@<href>{https://html5experts.jp/nakajmg/8931/,Googleのベストプラクティスに沿ったモダンな製作の出発点「Web Starter Kit」 | HTML5Experts.jp} を参考にしてみてください。
...
例えば「CSS で背景色をつけるやり方」と検索した時よりも「CSS 背景色 やり方」で検索したほうが検索結果の件数が増えます。



検索エンジンは「CSS で背景色をつけるやり方」のような文章を受け付けるのがまだ苦手です。そのため、文章よりも単語ごとに区切ったほうが検索結果の件数が増えます。


==== 「何々 とは」で検索する


より入門的な内容を探したい場合は「何々 とは」で検索することをおすすめします。例えば「JavaScript とは」や「CSS とは」といった感じです。



だいたい「何々 とは」というタイトルで入門的な記事を書く傾向にあるため、このように検索したほうがより入門よりの記事が引っかかります。
...
しかし @<href>{https://chrome.google.com/webstore/detail/add-search-options/kaifgnoibnjldmdkkfogenbfbfgcbiao?hl=ja,Add Search Options} という Google Chrome の拡張機能を使うことにより、期間指定に「三ヶ月以内」と「六ヶ月以内」が使えるようになります。
...
上から順に読んでいけば、仕事で JavaScript を触ることになっても十分に対処できると思います。
 ...
 1. @<href>{http://amzn.to/1I8kjCe,ノンプログラマのためのJavaScriptはじめの一歩}
 1. @<href>{http://amzn.to/1CL5dCa,開眼! JavaScript ―言語仕様から学ぶJavaScriptの本質}
 1. @<href>{http://amzn.to/1fagUYl,JavaScriptエンジニア養成読本}
 1. @<href>{http://amzn.to/1LetyDz,JavaScriptパターン ―優れたアプリケーションのための作法}
 1. @<href>{http://amzn.to/1Max5SF,Effective JavaScript}
...
しかし、それをインターネット上などで公開するために使ってしまった場合、@<href>{http://law.e-gov.go.jp/htmldata/S45/S45HO048.html#1000000000002000000003000000003000000000000000000000000000000000000000000000000,著作権法　第二十三条}の公衆送信権を侵害することになります。
...
この辺りの話は、@<href>{http://www.bengo4.com/houmu/17/1263/n_3282/,大人気ゲーム「刀剣乱舞」に「トレパク」疑惑が続々浮上――どんな問題があるのか？|弁護士ドットコムニュース}を見ると良いでしょう。
...
まず一つ目は、写真や配色や隙間を上手く使って、見た目が簡素でもセンスが良いサイトを作ることです。まず思い浮かぶのは @<href>{http://www.apple.com/jp/,Apple Japan} のサイトです。サイトの要素的にはそこまで多くないですが、写真と隙間を上手く使って、かっこ良いという印象を生み出しています。



また @<href>{https://www.google.com/nexus/,Nexus - Google} も Apple と同じくそこまで多くない要素を、大きな写真や隙間や影を効果的に使ってやはりかっこ良いという印象があります。
...
二つ目は、写真素材サイトを使うことです。商用利用可かつ、クレジット表記が不要という写真素材サイトは多くあります。有名なサイトとして、@<href>{https://www.pakutaso.com/,ぱくたそ}があります。また女性向けの写真素材サイトとしては、@<href>{http://girlydrop.com/,GIRLY DROP(ガーリードロップ)} というサイトがあります。



またこういった「商用利用可かつ、クレジット表記が不要」というサイトを横断的に検索できる@<href>{http://www.tadapic.com/,タダピク}というサイトもあります。
...
はじめに、そのライブラリが広く使われているかを見ます。例えば @<href>{http://getbootstrap.com/,Bootstrap} だと、ページ下部に Bootstrap を使って作られたサイト一覧が掲載されています。



また @<href>{http://jquery.com/,jQuery} については、@<href>{http://www.amazon.co.jp/s/field-keywords=jquery,Amazon で jQuery で検索}してみると、本が大量にでてきます。
...
例えば、ソースコードの全体が把握しやすいライブラリとしては、@<href>{http://backbonejs.org/,Backbone.js}が挙げられます。
...
しかし、@<href>{https://github.com/jashkenas/backbone/blob/master/backbone.js,ソースコード}はまとまりの単位が小さいため、1,877行(2015/7/10現在)あるとはいえ、全体を読もうと思えば読めなくはないものとなっています。



次に、ドキュメントが揃っていて読みやすい例としては、@<href>{http://jquery.com/,jQuery} が挙げられます。



ソースコード自体は @<href>{https://github.com/jquery/jquery/tree/master/src,リポジトリの src ディレクトリ配下}にまとまっており、機能ごとにまとまってはいるのですが、ファイル数が多く全体を把握しようとすると骨が折れます。



ただ、@<href>{https://api.jquery.com/,ドキュメント}が見やすくまとまっており、普段はソースコードを見なくてもどのように使うのか調べるのに困らないという状況になっています。



ちなみに、両方揃っている例としては @<href>{http://hokaccha.github.io/js-flipsnap/,flipsnap.js} が挙げられます。
...
また@<href>{https://github.com/hokaccha/js-flipsnap/blob/master/flipsnap.js,ソースコード自体}も、まとまりの単位が小さく、読みやすいものとなっています。
...
CSS を実装する際に使うライブラリとして、@<href>{https://github.com/twbs/bootstrap,Bootstrap} を使います。
 ...
 * @<href>{http://getbootstrap.com/css/#grid,グリッド機能}が使いやすい
 * ブラウザ間の既定の CSS を良い感じに丸め込んでくれる @<href>{http://necolas.github.io/normalize.css/,Normalize.css} が使える
 ...
 * @<href>{http://expo.getbootstrap.com/,Bootstrap Expo}
 * @<href>{https://openstf.github.io/,STF | Smartphone Test Farm}
 * @<href>{https://pplog.net/,pplog - ゆるふわインターネットにポエムを刻もう}
...
「カルーセル」は @<href>{http://www.nike.com/jp/ja_jp/,Nike} などで使われている「注目させたい複数の画像を次々に横にスライドさせて表示させる」ものです。



このカルーセルを実現するために、今回は @<href>{http://meandmax.github.io/lory/,lory} というライブラリを使います。この lory というライブラリを選んだ理由は以下の通りです。
...
なお、次々に横にスライドさせて表示させる際に、最後まで到達した時にまた最初まで戻る、無限ループがいらなければ @<href>{http://hokaccha.github.io/js-flipsnap/,flipsnap.js} もおすすめです。



また jQuery に依存するライブラリでは @<href>{http://kenwheeler.github.io/slick/,slick} や @<href>{http://owlgraphic.com/owlcarousel/,Owl Carousel} がよく使われます。
