...
今回の制作は、工数などを考えて、TOP ページとライブ中継ページの2ページだけにすることになります。
...
今回はすでにあるサイトを作ることなので、「要素や位置が決められた」状態で製作用の企画資料を作れば OK という、深く考えることがいらなくて比較的に楽な作業です。
...
また、TOP ページを構成する要素はグローバルメニュ以外、各ページの共通要素もあります。



ヘッダーにロゴ・メールマガジン・シェア・つぶやき・お問い合わせがあって、フッターには利用規約や権利情報表示などが掲載されています。
...
参加するアイドルグループがみる OUTLINE や SCHEDULE などより左に位置することは、設計者が参加者の他に、ライブをネット上で見るファンも大事にするからでしょう。



最初に書いた要件定義を見ると、他のページは今回全部作る必要がないため、実はここまで書いて OK です。どこまで資料が必要なのかは、要件定義で明確化されているため、余計な作業を増やして時間を無駄遣いすることがなくなるでしょう。
...
デザイン回避：ワイヤーフレームはあくまで要素がページ内にあることを示すための資料なので、デザインなどはなるべく入れないことをおすすめです。何故かと言うと、このワイヤーフレームを参考にデザイナーさんがよしなにグラフィックと UI を作ってくれますから。どんなデザインがいいのかは、アートディレクターの仕事なので、企画資料としては見やすいようにすることが最優先です。派手な色をつけたりいろんな形を使ったりすると、資料自体がわかりづらくなって、デザイナーさんのクリエイト活動に妨害する可能性もあります。
...
どう見て要素としてフッター内に入れる可能性が少ないと思います。
LIVE STAGE にもドットがフッター内にあるけど、生中継のページになってますので、実際のサイトでしたら映像にはこういう仕様も取らないはずだと思います。
...
ちょっと説明しますと、もともと「ニコニコ動画」みたいなコメント機能があると思われますが、今回は工数が足りないためちょっと改造して、「何かをコメントする」という似てる体験をユーザーに与えるため SNS コメントという形を取らせていただきました。
