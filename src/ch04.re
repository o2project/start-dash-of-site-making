...
セレクタとは HTML 内の特定要素に合致するパターンを指します。@<href>{http://standards.mitsue.co.jp/resources/w3c/TR/css3-selectors/#selectors,セレクタ Level 3 の 2. セレクタ}を見ると分かるように多種多様なセレクタがあります。
...
この場合 @<tt>{nav-bar} という値の class 属性を指定している div 要素からすると、ul 要素は子要素になります。また、li 要素は孫要素の関係になります。li 要素内の a 要素は曾孫要素になります。
...
前述した CSS は、文字色を赤にするよう定義しています。適用対象は @<tt>{nav-bar} というクラスが指定されている @<tt>{div} 要素の子要素である @<tt>{li} 要素になります。
...
たとえば次の HTML 内にある @<tt>{li} 要素全てへ対しては文字色を変更したくないという要望が来たとします。
...
この場合、次のように大なり括弧で @<tt>{li} 要素を指定すれば、トップとライブステージだけ文字色が赤くなります。
...
@<tt>{:hover} はある要素をマウスカーソルなどで示した場合に適用されるものです。
...
@<tt>{:active} はある要素を選択した時に適用されます。



例えば、マウスカーソルである要素を選択したとします。この場合、マウスから指を離すまでは @<tt>{:active} で指定されたスタイルが適用されます。



@<tt>{:focus} はある要素を選択した時に適用されます。



@<tt>{:active} と違うところは、入力フォームを選択した時にスタイルが適用されます。また、キーボードの tab キーでリンクを選択した時にスタイルが適用されるということです。
 ...
 * style 属性の数を @<tt>{a} として数える
 * ID セレクタの数を @<tt>{b} として数える
 * セレクタ内のクラスセレクタ(.foo)、属性セレクタ([id="foo"] など)、擬似クラス(:hover, :active など)の数を @<tt>{c} として数える
 * 型セレクタ(要素そのものを指定する)、擬似要素(::before, ::after など)の数は @<tt>{d} として数える
...
なお @<tt>{!important} を書くと、この優先度は関係なくなります。@<tt>{!important} は強力すぎるため、限られた場所のみ使うのがいいでしょう。
