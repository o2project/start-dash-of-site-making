...
この場合 @<tt>{nav-bar} という値のclass属性を指定しているdiv要素からすると、ul要素は子要素になります。
...
前述したCSSは、文字色を赤にするよう定義しています。適用対象は @<tt>{nav-bar} というクラスが指定されている @<tt>{div} 要素の子要素である @<tt>{li} 要素になります。
...
たとえば次のHTML内にある @<tt>{li} 要素全てへ対しては文字色を変更したくないという要望が来たとします。
...
この場合、マウスから指を離すまでは @<tt>{:active} で指定されたスタイルが適用されます。



@<tt>{:focus} はある要素を選択した時に適用されます。
@<tt>{:active} と違うところは、入力フォームを選択した時にスタイルが適用されます。
 ...
 * style属性の数を @<tt>{a} として数える
 * IDセレクタの数を @<tt>{b} として数える
 * セレクタ内のクラスセレクタ（@<tt>{.foo}）、属性セレクタ（例：@<tt>{[id="foo"]}）、擬似クラス（例：@<tt>{:hover}）の数を @<tt>{c} として数える
 * 型セレクタ（要素そのものを指定する）、擬似要素（@<tt>{::before}, @<tt>{::after}など）の数は @<tt>{d} として数える
